...
function findWasmBinary() {
  ...
  if (Module['locateFile']) {
    return locateFile('lumen.wasm');
  }

  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL('lumen.wasm', import.meta.url).href;
...
function __asyncjs__js_request_input() { return Asyncify.handleAsync(async () => { let value; if (typeof Module['lumenGetInput'] === 'function') { value = await Module['lumenGetInput'](); } else { value = window.prompt('Input:') ?? ''; } const len = lengthBytesUTF8(value) + 1; const ptr = _malloc(len); stringToUTF8(value, ptr, len); return ptr; }); }

// Imports from the Wasm binary.
var ___getTypeName = makeInvalidEarlyAccess('___getTypeName');
var _getLmnbin = Module['_getLmnbin'] = makeInvalidEarlyAccess('_getLmnbin');
var _getLmnbinSize = Module['_getLmnbinSize'] = makeInvalidEarlyAccess('_getLmnbinSize');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['getLmnbin'] != 'undefined', 'missing Wasm export: getLmnbin');
  assert(typeof wasmExports['getLmnbinSize'] != 'undefined', 'missing Wasm export: getLmnbinSize');
  ...
  _getLmnbin = Module['_getLmnbin'] = createExportWrapper('getLmnbin', wasmExports['getLmnbin'], 0);
  _getLmnbinSize = Module['_getLmnbinSize'] = createExportWrapper('getLmnbinSize', wasmExports['getLmnbinSize'], 0);
  ...
  dynCall_iiijiii = dynCalls['iiijiii'] = createExportWrapper('dynCall_iiijiii', wasmExports['dynCall_iiijiii'], 7);
